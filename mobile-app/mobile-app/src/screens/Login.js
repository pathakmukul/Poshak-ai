...
function Login({ onLogin }) {
  ...
  // Commented out since users already exist in Firebase emulator
  // useEffect(() => {
  ...
  const dummyUsers = DUMMY_USERS ? DUMMY_USERS.map(user => user.displayName) : [];
  console.log('Mapped dummy users:', dummyUsers);
          ...
          <View style={styles.form}>
            ...
            <View style={styles.pickerContainer}>
              <Picker
                ...
                itemStyle={{ color: '#fff' }}
              >
                <Picker.Item label="-- Choose a user --" value="" color="#999" />
                {dummyUsers.map(user => (
                  <Picker.Item key={user} label={user} value={user} color="#fff" />
                ))}
              </Picker>
            </View>
